...
interface Props {
  ...
  onTargetMultiplierChange: (v: number) => void;
}
...
function TargetMultiplierPicker({ value, onChange }: TargetPickerProps) {
  const customId = useId();
  ...
  function activateCustom() {
    setCustomActive(true);
    const initial = matchesPreset ? "1.5" : String(value.toFixed(1));
    setCustomStr(initial);
    const n = parseFloat(initial);
  ...
  return (
    <div className="flex flex-col gap-2">
      <label className="text-sm font-medium text-gray-300">Meta de jubilación</label>
      <p className="text-xs text-gray-500">
        Con cuántos salarios mínimos querés jubilarte? (en valores de hoy como referencia)
      </p>
      <div className="flex flex-wrap gap-2">
        {presets.map((p) => {
          const active = !customActive && Math.abs(p - value) < 1e-5;
          return (
            <button
              key={p}
              type="button"
      ...
      {customActive && (
        <div className="flex items-center gap-2 mt-1">
          ...
          <input
            ...
            className="w-20 rounded border border-gray-600 bg-gray-800 px-3 py-1.5 text-sm text-gray-100 focus:border-blue-500 focus:outline-none"
          />
          <span className="text-sm text-gray-500">x sal. mínimo</span>
        </div>
      )}
    </div>
  );
